...
const SquareButton = ({ children, onClick, className }: any) => {
  return (
    <button onClick={onClick} className={`relative w-fit cursor-pointer group ${className}`}>
      <div className="bg-primary z-[3] relative h-full group-hover:bg-secoundry border border-secoundry group-hover:text-white flex justify-center flex-col items-center">{children}</div>
